...
export type MatchResult = {
  ...
  breakdown: string[];     // short human-readable reasons
...
// ---------- Public: computeMatch (shim used by /invest) ----------
export function computeMatch(
  pitch: Partial<Pitch> | (Partial<Pitch> & Record<string, unknown>),
  _prefs?: Record<string, unknown>, // optional; reserved for future tuning
): MatchResult {
  const o = (pitch ?? {}) as Record<string, unknown>;
  ...
  // Momentum
  score += Math.min(25, Math.floor(fundedPct / 2)); // up to +25
  ...
  if (minInvestment > 0) {
    ...
    score += bump;
    breakdown.push(`Minimum investment ${minInvestment.toLocaleString('en-US', {
      style: 'currency',
      currency: 'USD',
      maximumFractionDigits: 0,
    })} (+${bump})`);
